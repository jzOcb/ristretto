...
use rist_shared::{
    AgentInfo, AgentType, EventFilter, MergeStrategy, SessionId, Task,
};
...
impl DaemonClient {
    ...
    pub async fn connect(path: PathBuf) -> io::Result<Self> {
        ...
        if !is_valid_subscribe_response(&initial_response) {
...
async fn take_or_connect(
    ...
    .await?;
    let response: Response = decode_frame_async(&mut stream).await?;
    if !is_valid_subscribe_response(&response) {
        return Err(io::Error::new(
            io::ErrorKind::InvalidData,
            "unexpected subscribe response",
        ));
...
fn is_valid_subscribe_response(response: &Response) -> bool {
    matches!(response, Response::Ok | Response::Unknown)
}

fn decode_frame_kind(value: Value) -> io::Result<FrameKind> {
    let frame_type = value
        .get("type")
        .and_then(Value::as_str)
        .ok_or_else(|| io::Error::new(io::ErrorKind::InvalidData, "missing frame type"))?;

    if is_event_type(frame_type) {
        serde_json::from_value(value)
            .map(FrameKind::Event)
            .map_err(|error| io::Error::new(io::ErrorKind::InvalidData, error))
    } else {
        serde_json::from_value(value)
            .map(FrameKind::Response)
            .map_err(|error| io::Error::new(io::ErrorKind::InvalidData, error))
    }
}

fn is_event_type(frame_type: &str) -> bool {
    matches!(
        frame_type,
        "pty_data"
            | "status_change"
...
#[cfg(test)]
mod tests {
    use rist_shared::protocol::Response;

    use super::is_valid_subscribe_response;

    #[test]
    fn subscribe_accepts_ok_and_unknown() {
        assert!(is_valid_subscribe_response(&Response::Ok));
        assert!(is_valid_subscribe_response(&Response::Unknown));
    }

    #[test]
    fn subscribe_rejects_error() {
        assert!(!is_valid_subscribe_response(&Response::Error {
